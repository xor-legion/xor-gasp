...
def evaluate(code,raw_input=None,max_ticks = None):
    ...
    if len(bracemap.keys()) % 2 != 0:
        raise Exception("Failed brace mapping")
    ...
    while codeptr < len(code):
        ...
        if debug: print("CMD/CDPTR/CLPTR: ",command,codeptr,cellptr,cells[cellptr],chr(cells[cellptr]))
...
def cleanup(code):
    return filter(lambda x: x in list(bf_tokens), code)
